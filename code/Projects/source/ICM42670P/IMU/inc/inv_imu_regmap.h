 ...
 * â€?Write FIFO_FLUSH =1
 * â€?Wait for 1.5 Âµs
 * â€?Read FIFO_FLUSH, it should now be 0
 ...
 * 
 * Gyroscope needs to be kept ON for a minimum of 45ms. When transitioning from OFF to any of the other modes, do not issue any register writes for 200 Âµs.
 * 
 ...
 * 
 * The recommended setting of this bit is â€?â€?before entering and during all power modes excluding ALP with WUOSC. This is in order to avoid having to do a FIFO access/flush before entering sleep mode. During ALP with WUOSC it is recommended to set this bit to â€?â€? It is recommended to reset this bit back to â€?â€?before exiting ALP+WUOSC with a wait time of 1 ODR or higher.
 */
 ...
 * 1 Tag Fsync flag to TEMP_OUTâ€™s LSB
 * 2 Tag Fsync flag to GYRO_XOUTâ€™s LSB
 * 3 Tag Fsync flag to GYRO_YOUTâ€™s LSB
 * 4 Tag Fsync flag to GYRO_ZOUTâ€™s LSB
 * 5 Tag Fsync flag to ACCEL_XOUTâ€™s LSB
 * 6 Tag Fsync flag to ACCEL_YOUTâ€™s LSB
 * 7 Tag Fsync flag to ACCEL_ZOUTâ€™s LSB
 */
 ...
 * WoM thresholds are expressed in fixed â€œmgâ€?independent of the selected Range [0g : 1g]; Resolution 1g/256=~3.9mg
 */
